...
const UserListPage = () => {
    ...
    const queryParams = useMemo(() => {
        return new URLSearchParams({
            ...
            ...(searchParams.productPrice && { productPrice: searchParams.productPrice })
        }).toString();
    }, [page, searchParams]);
    ...
    const { data, error, isLoading } = useSWR<tableDataResponse>(`http://localhost:80/product/list?${queryParams}`, swrFetcher);
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            const res = await swrFetcher(`http://localhost:80/product/delete/${id}`, {
                headers: {
            ...
            await mutate(`http://localhost:80/product/list?${queryParams}`);
        } catch (err) {
            console.error(err);
    ...
    const handleBatchDelete = async (ids: string[]) => {
        ...
        try {
            await Promise.all(
                ids.map(id =>
                    fetch(`http://localhost:80/product/delete/${id}`, {
                        headers: {
                            'Authorization': `Bearer ${token}`,
                            'Content-Type': 'application/json',
                        },
    ...
    const handleExcelDownload = async () => {
        try {
            ...
            const res = await swrFetcher(`http://localhost:80/product/excel?${queryParams}`, {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${token}`,
                },
