...
const ListPage = () => {
    ...
    const { data, error, isLoading } = useSWR<tableDataResponse>(`/${currentLocale}/api/member/list?${queryParams}`, fetcher);
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            const res = await swrFetcher(`/${currentLocale}/api/admin/member/delete/${id}`, {
                headers: {
            ...
            await mutate(`/${currentLocale}/api/admin/member/list?${queryParams}`);
        } catch (err) {
            console.error(err);
    ...
    const handleBatchDelete = async (ids: string[]) => {
        ...
        try {
            await Promise.all(
                ids.map(id =>
                    fetch(`/${currentLocale}/api/admin/member/delete/${id}`, {
                        headers: {
                            'Authorization': `Bearer ${token}`,
                            'Content-Type': 'application/json',
                        },
    ...
    const handleExcelDownload = async () => {
        try {
            ...
            const res = await swrFetcher(`/${currentLocale}/api/admin/member/excel?${queryParams}`, {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${token}`,
                },
