...
const Table: React.FC<TableProps> = ({ locale, title, role,  totalCount, theaders, contents, details, onDelete, onEdit, currentPage, setCurrentPage, onBatchDelete, selectedDatas, setSelectedDatas, handleExcelDownload  }) => {
    ...
    // contents를 빈 배열로 기본값 설정
    const safeContents = Array.isArray(contents) ? contents : [];
    const primaryKey = safeContents.length > 0 ? Object.keys(safeContents[0])[0] : '';
    ...
    const sortedUsers = contents ? [...contents].sort((a, b) => {
    ...
    const totalPages = details?.totalPages ? details.totalPages : 0;
            ...
            <div className="p-6 border-b border-gray-200">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between">
                    ...
                    <div className="mt-4 md:mt-0 flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-4">
                        ...
                        <button
                            onClick={() => router.push(`/${locale}/admin/${role}/edit/new`)}
                            type="button"
            ...
            <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        {Array.from(theaders.entries()).map(([key, value]) => (
                            <th
                                    key={key}
                                    scope="col"
                    ...
                    {currentPageData.length === 0 ? (
                        <tr>
                            <td colSpan={theaders.size+3} className="text-center px-6 py-4 text-gray-500">
                                데이터가 없습니다.
                            </td>
                        </tr>
                    ) : (
                        currentPageData.map((column) => {
                            const values = Object.values(column) as any[];// 객체의 value 배열
                            ...
                            return (
                                <tr key={values[0].toString()} className={selectedDatas.includes(values[0].toString()) ? 'bg-blue-50' : ''}>
                                    ...
                                    {Array.from(theaders.entries()).map(([h_key, value], index) => (
                                        <td key={h_key} className="px-6 py-4 whitespace-nowrap max-w-[400px]">
                                            <div
                                                className="text-sm font-medium text-gray-900 truncate"
                                                title={h_key === "productPrice" ? numberWithCommas(values[index + 1]) : values[index + 1]} // tooltip
                                            >
                                                {h_key === "productPrice"
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div className="flex space-x-2">
                                            <button
                                                onClick={() => router.push(`/${locale}/admin/${role}/edit/${values[0]}`)}
