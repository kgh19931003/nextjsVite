...
const Form = ({ locale, idx }: { locale: string; idx?: string }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const res = await swrFetcher<AlloyResponse>(`/${currentLocale}/api/admin/alloy/one/${idx}`);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            alert(isEditMode ? '강종 수정 완료' : '강종 추가 완료');
            await mutate(`/${currentLocale}/api/admin/alloy/list?page=1`);
            router.push(`/${currentLocale}/admin/alloy/list`);
        } catch {
            alert('저장 실패');
        } finally {
            setLoading(false);
    ...
    return (
        <div className="max-w-5xl mx-auto p-6 bg-white shadow-md rounded-md">
            ...
            <div className="space-y-4">
                ...
                <div className="flex-1">
                    <label className="block mb-1 text-sm font-medium text-gray-700">타입</label>
                    <input
                        type="text"
                        name="type"
                        value={form.type}
